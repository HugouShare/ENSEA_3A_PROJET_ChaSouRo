...
// Ajout FreeRTOS
extern SemaphoreHandle_t sem_select_menu;
extern SemaphoreHandle_t sem_enter_menu;
...
//	uint32_t now = HAL_GetTick();
//
//	    if (Flags.Select_Menu == 1) {
//	    	if (now - lastPressBtnPA1 > 100) {
//
//	    	lastPressBtnPA1 = now;
//	    	lastMenuActivity = now; // Pour reset le compteur de la veille sinon s'active trop
//
//	    	HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_15);
//	    	switch (currentMenu) {
//
...
//	    	Flags.Select_Menu = 0;
//	    }
//	    if (Flags.Enter_Menu == 1){
//	    	if (now - lastPressBtnPB9 > 100) {
//
//				lastPressBtnPB9 = now;
//				lastMenuActivity = now;
//
